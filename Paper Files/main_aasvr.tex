...
Public process datasets provide independent stress tests for measurement validation. HAI 21.03 contains industrial-control attack data from a hardware-in-the-loop testbed \cite{shin2021haicon}. SKAB contains multivariate process-loop experiments with anomaly and changepoint labels \cite{skab2020}. SWaT, WaDi, and Tennessee Eastman are relevant larger benchmarks for future validation, subject to access and redistribution constraints \cite{swatDataset,wadiDataset,tepDataset}.
...
Table~\ref{tab:hydro_synth} summarizes the primary target-domain result on held-out synthetic-fault trials. LOF and AASVR are effectively tied on balanced accuracy in operational terms, although LOF is fractionally higher on the point-wise detector score. They reach that point through different tradeoffs: LOF has higher recall and lower specificity, whereas AASVR has much lower false actuation under the shared supervisor. This is the intended comparison after the reviewer-suggested fairness correction: the detector component is competitive, while the authorization layer produces the operational reduction in unnecessary actuation. The headline test balanced accuracy is slightly below the best validation-split value in Table~\ref{tab:tuning} because the two tables use disjoint synthetic-fault splits.
...
\midrule
LOF & 0.853 & 0.798 & \best{0.826} & 0.202 & 0.270 & 2.811 & 4.240 \\
AASVR & 0.777 & 0.871 & 0.824 & 0.129 & \best{0.000} & \best{0.259} & \best{0.853} \\
...
One-Class SVM & \best{0.912} & 0.333 & 0.623 & 0.667 & 0.089 & 2.796 & 4.227 \\
...
\midrule
LOF & -0.002 & 0.0025 & 2.552 & [2.077, 3.026] & $<0.001$ \\
Isolation Forest & 0.121 & $<0.001$ & 2.648 & [2.192, 3.107] & $<0.001$ \\
...
\midrule
CO2 & \best{0.849} & \best{0.884} & \best{0.867} & 0.259 \\
Water temperature & 0.820 & 0.873 & 0.847 & 0.222 \\
Air temperature & 0.822 & 0.863 & 0.842 & 0.222 \\
pH & 0.785 & 0.870 & 0.828 & \best{0.019} \\
EC & 0.609 & 0.865 & 0.737 & 0.574 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:faulttype} and Figure~\ref{fig:fault_heatmap} show performance by injected fault class. AASVR performs best on saturation, dropout, bias, spike, noise burst, step, and multi-spike faults. Drift and stuck-at faults remain difficult because they can resemble slow process movement or repeated steady values without an independent reference. The added stuck-value guard improves stuck-at balanced accuracy only modestly, from 0.465 without the guard to 0.474 with the guard, so it should be read as a mitigation rather than a solved failure mode. False actuation is also concentrated in the stuck-at case: the overall AASVR median is zero false actuations per trial, but stuck-at faults average 1.333 false actuations per trial with a median of 1.0.
...
\caption{Hydroponic synthetic-fault heatmap by sensor and fault type. The figure summarizes where AASVR is strongest and where slow drift and stuck-at behavior remain limitations.}
\label{fig:fault_heatmap}
\end{figure}
\FloatBarrier

\subsection{Ablation and control effect}
Table~\ref{tab:ablation} shows that several ablations preserve similar detection scores but change control behavior. Removing actuation confirmation increases false actuation from 0.233 to 0.583 events per trial in the sampled held-out ablation set, and removing cooldown increases it to 0.475. The ablation uses a deterministic 120-trial subset of the held-out test split for tractability, which explains the small difference from the full-test result in Table~\ref{tab:hydro_synth}. The stuck-detector contrast is better interpreted through the stuck-at-specific result in Table~\ref{tab:faulttype}; its aggregate effect is small because stuck-at is one of several injected fault classes.
...
The validation sweep in Table~\ref{tab:tuning} shows that the selected AASVR configuration is not an isolated optimum. Multiple trust-score and transient-limit settings produced the same validation objective when the robust scale multiplier was 5.0. These parameters were selected before evaluating the held-out test split in Table~\ref{tab:hydro_synth}. A post hoc consistency check showed that all four tied validation settings produced the same held-out test balanced accuracy and false-actuation mean, so the reported test result is not sensitive to which tied setting is chosen.
