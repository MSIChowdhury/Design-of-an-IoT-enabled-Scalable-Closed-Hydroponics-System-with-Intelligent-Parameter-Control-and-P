...
\item a reproducible Docker and LaTeX workflow that regenerates the tables, figures, and diagnostic outputs given the local deployment data, with curated hydroponic release planned on acceptance.
\end{enumerate}
...
\begin{table}[!htbp]
\centering
\caption{Compact synthetic fault-injection protocol for the current hydroponic replay benchmark. The repository also defines a larger shardable stress-test profile, but those full-factorial results are not used as manuscript evidence.}
...
\begin{table}[!htbp]
\centering
\caption{Held-out hydroponic synthetic-fault benchmark. Baselines are validation-tuned under the corrected objective in Eq.~\eqref{eq:validation_objective} and evaluated on the same test split, so values may differ from earlier internal reports. Missed authorization is reported only for AASVR variants because it depends on their trusted-state unsafe-band trace; lower is better for false actuation and missed authorization.}
...
The base-AASVR comparison is a three-way replay tradeoff. AASVR-R has a small but highly consistent balanced-accuracy decrement relative to base AASVR ($\Delta$BA $=-0.001$, standard deviation about 0.0018), a higher missed-authorization rate, and a lower false-authorized-actuation count. The negative detection sign is therefore stated explicitly rather than treated as an advantage; the operational contrast is whether the false-authorized-actuation reduction is worth the additional authorization suppression. The LOF balanced-accuracy difference is statistically detectable but operationally small ($d_z=0.113$). Its practical contrast is the control profile: AASVR-R has higher specificity, fewer alerts, and lower false-authorized actuation. For One-Class SVM, the false-actuation contrast is not distinguishable; the separation is instead its aggressive rejection and low specificity.
...
Table~\ref{tab:component} separates the contribution of the supervisory components on the same 270 held-out test trials as Table~\ref{tab:hydro_synth}. Lockout-only control has no fault detection and still produces 1.356 false-authorized actuations per trial, so the low AASVR-R false-actuation rate is not explained by the existing confirmation/cooldown rule alone. The table also shows the central tradeoff: reducing replay false actuation by adding response reliability increases the missed-authorization rate relative to base AASVR. The trust score is retained as an audit and authorization diagnostic, not as an independent detector contribution; in the selected configuration, hard gates and persistence dominate until the score is coupled to the full authorization logic.
...
The repository contains code, configuration files, processed-data descriptors, Docker workflow files, and paper-generation assets. Raw hydroponic and external benchmark files are not committed to the repository during review. The authors plan to release the curated hydroponic feed and non-sensitive actuator-command evidence package on acceptance, subject to removal of credentials and private operational fields. Until then, the computational workflow is reproducible given local access to those data. Public datasets can be obtained from their original providers, and request-access datasets should be obtained under the providers' terms.
