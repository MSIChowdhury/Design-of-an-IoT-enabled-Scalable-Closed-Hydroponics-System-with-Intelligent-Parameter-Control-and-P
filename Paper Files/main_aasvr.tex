...
Figure~\ref{fig:operating_tradeoff} expands this point across authorization operating settings. The grid varies trust threshold, authorization persistence, replay cooldown, and response-reliability risk thresholds while keeping the held-out fault windows fixed. The manuscript setting is intentionally conservative: it sits near the low replay-false-authorized-actuation region, but it does so by accepting a high missed-authorization rate. This figure is included to make the operating tradeoff explicit rather than optimizing only for one favorable error direction.
...
The repository contains code, configuration files, processed-data descriptors, Docker workflow files, paper-generation assets, and optional evidence schemas for future deployments. Raw hydroponic and external benchmark files are not committed to the repository during review. The authors plan to release the curated hydroponic sensor feed on acceptance, subject to removal of credentials and private operational fields. Timestamped actuator-command/state evidence is not available for the historical deployment; future deployments should retain it using the schema supplied with the reproducibility package. Until release, the computational workflow is reproducible given local access to the data. Public datasets can be obtained from their original providers, and request-access datasets should be obtained under the providers' terms.
