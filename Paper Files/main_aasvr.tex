...
\begin{abstract}
Closed-loop hydroponic systems can convert transient sensor faults into unnecessary dosing, cooling, pumping, or carbon dioxide actuation. This paper presents AASVR, an actuation-aware sensor validation and rectification layer for fault-resilient hydroponic control. AASVR combines robust range/rate gates, uncommanded-trend checks, persistence states, bounded rectification, and actuation authorization. On held-out hydroponic replay with synthetic faults, AASVR achieved 0.826 balanced accuracy and 0.267 false actuations per trial, matching the strongest detector while reducing false actuation by an order of magnitude. Public HAI and SKAB process datasets are used as external stress tests. The results support actuation-aware validation as a practical reliability layer for low-cost closed-loop automation.
\end{abstract}
...
\end{aligned}
\end{equation}
where the terms respectively measure anomalous pass-through, false rejection, false actuation, missed actuation, detection delay, residence outside safe bands, and computational cost. In the implemented validation sweep, the reported scalar objective uses normalized benchmark outputs with weights $w_{\mathrm{BA}}=1.0$, $w_{\mathrm{false\,act}}=-0.01$, $w_{\mathrm{false\,alarm}}=-0.0005$, and $w_{\mathrm{delay}}=-0.001$; the other terms are logged but not used for parameter selection in the current experiments.
...
HAI 21.03 and SKAB are used as public external stress tests. HAI labels are timestamp-level attack indicators over an industrial-control testbed. SKAB labels are timestamp-level anomaly indicators over process-loop experiments. For these datasets, a timestamp is predicted abnormal when the fraction of rejecting sensor streams exceeds an aggregation threshold. Thresholds are selected on the first half of each native-label series and reported on the second half; fixed-threshold sensitivity from 10\% to 90\% is also retained as diagnostic evidence.
...
The baselines are raw thresholding, the original manuscript rule, moving average, moving median, Hampel, Kalman, EWMA, CUSUM, PCA-style monitoring, GLR, recursive PCA, Isolation Forest, One-Class SVM, and local outlier factor. Every baseline is tuned on the validation split using the same control-aware objective family as AASVR and then wrapped in the same persistence/cooldown authorization supervisor for false-actuation scoring. This separates detector quality from downstream authorization. The hydroponic synthetic-fault benchmark reports precision, recall, specificity, balanced accuracy, event recall, detection delay, false alarm events, false actuation mean, and false actuation standard deviation. Native-label external benchmarks report timestamp-level precision, recall, specificity, and balanced accuracy.
...
Table~\ref{tab:hydro_synth} summarizes the primary target-domain result on held-out synthetic-fault trials. LOF and AASVR are effectively tied on balanced accuracy, but they reach that point through different tradeoffs: LOF has higher recall and lower specificity, whereas AASVR has much lower false actuation under the shared supervisor. This is the intended comparison after the reviewer-suggested fairness correction: the detector component is competitive, while the authorization layer produces the operational reduction in unnecessary actuation.
...
\midrule
LOF & 0.853 & 0.798 & \best{0.826} & 0.202 & 2.811 \\
AASVR & 0.771 & 0.880 & \best{0.826} & 0.120 & \best{0.267} \\
Kalman & 0.613 & 0.803 & 0.708 & 0.197 & 0.515 \\
Isolation Forest & 0.581 & 0.826 & 0.703 & 0.174 & 2.907 \\
Hampel & 0.411 & 0.984 & 0.698 & 0.016 & 1.041 \\
Moving median & 0.411 & 0.984 & 0.698 & 0.016 & 0.993 \\
Recursive PCA & 0.317 & 0.988 & 0.652 & 0.012 & 1.319 \\
One-Class SVM & \best{0.912} & 0.333 & 0.623 & 0.667 & 2.796 \\
GLR & 0.129 & \best{0.999} & 0.564 & \best{0.001} & 1.330 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Bootstrap uncertainty for held-out hydroponic synthetic-fault balanced accuracy. Intervals are computed by resampling injected-fault trials. The result should be read together with the paired tests because AASVR and LOF are close on point-wise balanced accuracy, while false-actuation behavior differs strongly.}
\label{fig:hydro_ci}
\end{figure}
\FloatBarrier
...
\midrule
CO2 & \best{0.843} & \best{0.892} & \best{0.868} & 0.259 \\
Water temperature & 0.809 & 0.882 & 0.846 & 0.241 \\
Air temperature & 0.818 & 0.872 & 0.845 & 0.241 \\
pH & 0.778 & 0.877 & 0.827 & \best{0.019} \\
EC & 0.606 & 0.879 & 0.742 & 0.574 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Saturation & \best{1.000} & 0.874 & \best{0.937} & \best{0.000} \\
Dropout & \best{1.000} & 0.870 & 0.935 & \best{0.000} \\
Bias & 0.910 & 0.879 & 0.895 & 0.233 \\
Spike & 0.896 & \best{0.890} & 0.893 & 0.167 \\
Noise burst & 0.893 & 0.885 & 0.889 & 0.133 \\
Step & 0.874 & 0.889 & 0.882 & 0.233 \\
Multi-spike & 0.852 & 0.877 & 0.865 & \best{0.000} \\
Drift & 0.463 & 0.876 & 0.670 & 0.300 \\
Stuck-at & 0.049 & 0.882 & 0.465 & 1.333 \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:ablation} shows that several ablations preserve similar detection scores but change control behavior. Removing actuation confirmation increases false actuation from 0.233 to 0.650 events per trial in the sampled held-out ablation set. This supports the central interpretation: point-wise detection is only part of the result, while actuation authorization determines the operational false-actuation reduction.

\begin{table}[t]
\centering
\caption{AASVR ablation summary on sampled held-out test trials. Detection scores can remain similar while control outcomes change. In particular, removing actuation confirmation leaves balanced accuracy essentially unchanged but increases false actuation, which supports evaluating control-aware metrics rather than point-wise detection alone.}
\label{tab:ablation}
\begin{tabular}{lrrr}
\toprule
Variant & Bal. acc. & Recall & False act. \\
\midrule
No robust MAD scale & \best{0.820} & \best{0.790} & \best{0.192} \\
No actuation confirmation & 0.819 & 0.757 & 0.650 \\
No cooldown & 0.819 & 0.757 & 0.467 \\
Full AASVR & 0.819 & 0.757 & 0.233 \\
No trend guard & 0.804 & 0.720 & 0.325 \\
Bounded prediction mode & 0.774 & 0.662 & 0.583 \\
\bottomrule
\end{tabular}
\end{table}

The validation sweep in Table~\ref{tab:tuning} shows that the selected AASVR configuration is not an isolated optimum. Multiple trust-score and transient-limit settings produced the same validation objective when the robust scale multiplier was 5.0. These parameters were selected before evaluating the held-out test split in Table~\ref{tab:hydro_synth}.

\begin{table}[!htbp]
\centering
\caption{Top AASVR parameter settings from the validation split. The selected setting uses $c_s=5.0$, $q_{\min}=0.70$, and transient limit 2; tied settings are retained in the table to show sensitivity.}
\label{tab:tuning}
\begin{tabular}{rrrrr}
\toprule
$c_s$ & $q_{\min}$ & Transient limit & Bal. acc. & Objective \\
\midrule
5.0 & 0.85 & 3 & \best{0.827} & \best{0.750} \\
5.0 & 0.85 & 2 & \best{0.827} & \best{0.750} \\
5.0 & 0.70 & 3 & \best{0.827} & \best{0.750} \\
5.0 & 0.70 & 2 & \best{0.827} & \best{0.750} \\
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.66\linewidth]{figure/hydro_exp1_ph_trace.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.66\linewidth]{figure/hydro_exp1_co2_trace.pdf}
...
Table~\ref{tab:external} reports native-label external stress tests using thresholds selected on validation timestamps and evaluated on held-out test timestamps. These datasets are included after the hydroponic results because they are supporting stress tests, not replacements for the target system. The results are intentionally reported conservatively: HAI and SKAB expose the difficulty of transferring a hydroponic-oriented validation layer to cyber-physical attack labels and process-loop anomaly labels without dataset-specific modeling.

\begin{table}[!htbp]
\centering
\caption{External native-label test results with aggregation thresholds selected on validation timestamps. These values are cross-domain stress tests rather than direct hydroponic fault evidence. HAI contains cyber-physical attack labels, while SKAB contains process-loop anomaly labels.}
\label{tab:external}
\begin{tabular}{llrrrr}
\toprule
Dataset & Method & Precision & Recall & Specificity & Bal. acc. \\
\midrule
HAI & Raw threshold & 0.000 & 0.000 & \best{0.994} & \best{0.497} \\
HAI & CUSUM & \best{0.004} & \best{0.528} & 0.439 & 0.484 \\
HAI & AASVR & 0.003 & 0.360 & 0.485 & 0.422 \\
HAI & Kalman & 0.002 & 0.146 & 0.630 & 0.388 \\
SKAB & AASVR & \best{0.356} & \best{1.000} & 0.000 & \best{0.500} \\
SKAB & Kalman & \best{0.356} & \best{1.000} & 0.000 & \best{0.500} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.72\linewidth]{figure/external_aasvr_threshold_sensitivity.pdf}
\caption{Threshold sensitivity for timestamp-level external labels on held-out test timestamps. The plot shows how AASVR changes when the required fraction of rejecting streams is varied. The weak held-out HAI/SKAB behavior is treated as a limitation rather than as a leaderboard claim.}
\label{fig:threshold}
\end{figure}
...
The ablation and baseline results show why actuation-aware validation should be evaluated with control metrics. The detection component is competitive with tuned baselines on held-out synthetic faults, but the authorization layer is what produces the operationally important false-actuation reduction. For hydroponic systems, this distinction is practical because unnecessary acid/base dosing, nutrient dosing, CO2 release, or cooling can disturb the process even if the signal-level anomaly score appears acceptable.

\subsection{Cross-domain generalization}
The external benchmark results support only a cautious generalization claim. AASVR can run on independent industrial-control and process-loop datasets with the same streaming interface, but validation-selected threshold results on HAI and SKAB are weak and should not be interpreted as dominance over process-specific methods. HAI attacks and SKAB process anomalies are not identical to low-cost hydroponic sensor faults. The external results therefore test transferability of the interface and reveal limits of direct parameter transfer, not agronomic performance.

\subsection{Limitations}
The hydroponic experiment does not provide independent reference instruments for every sensor at every time point. Synthetic fault injection is therefore necessary for controlled scoring, but injected faults cannot cover every real failure mode. Slow drift, stuck-at behavior, and EC-bias cases remain the principal weaknesses because they can resemble real nutrient-solution dynamics without cross-sensor or model-based residual evidence. Addressing these cases likely requires a stronger process model, observer residual, or cross-sensor consistency check. The external datasets use timestamp-level labels, so the multivariate aggregation threshold affects the result. Water level is excluded from primary analysis because the available raw values are not defensibly calibrated. Crop outcomes are treated only as deployment context because enclosure, lighting, cooling, nutrient delivery, carbon dioxide regulation, and automation are confounded.

\section{Conclusion}
We presented a closed-loop hydroponic control system and proposed AASVR as its actuation-aware sensor validation and rectification layer. The method separates validation, state update, rectification, and actuation authorization so that untrusted measurements can be logged without directly driving physical commands. On held-out hydroponic synthetic faults, AASVR was competitive with the strongest tuned detector while producing much lower false actuation. Public SKAB and HAI benchmarks show that the same interface can be applied to external process data, but also expose threshold-dependent and domain-transfer limitations that should be reported rather than hidden.
