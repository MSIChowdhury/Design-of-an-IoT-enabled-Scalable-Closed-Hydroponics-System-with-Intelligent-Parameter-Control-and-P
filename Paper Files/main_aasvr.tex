...
We define a measurement as \emph{untrusted} when it is missing, physically implausible, rate-inconsistent, inconsistent with recent trusted state, or inconsistent with expected actuator effects. In replay, we define a \emph{false-authorized actuation} as an authorization caused by untrusted evidence. A \emph{missed authorization} is a failure to authorize action when the trusted state remains outside the control band.
...
\begin{table}[!htbp]
\centering
\caption{Evidence status for the current revision. The table separates available deployment evidence from prospective evidence needed to convert replay checks into measured closed-loop actuator-response claims.}
...
\begin{table}[!htbp]
\centering
\caption{Paired held-out effects for AASVR-R against representative methods. Positive $\Delta$BA means AASVR-R has higher per-trial balanced accuracy; positive $\Delta$replay false authorization means the comparison method has more replay false-authorized actuations than AASVR-R. $d_z$ is the paired standardized BA effect size; $p_{\mathrm{FA,sign}}$ is Holm-corrected for the sign-test replay-authorization family.}
\label{tab:paired_effects}
\footnotesize
\resizebox{\linewidth}{!}{%
\begin{tabular}{lrrrrrr}
\toprule
Baseline & $\Delta$BA & $d_z$ & $p_{\mathrm{BA}}$ & $\Delta$Replay false auth. & 95\% CI & $p_{\mathrm{FA,sign}}$ \\
\midrule
...
\begin{table}[t]
\centering
\caption{AASVR-R synthetic-fault performance by hydroponic sensor. Each sensor row contains 54 held-out trials; replay false authorization is reported as mean events per injected-fault trial.}
...
\midrule
Healthy diagnostic pulse & \best{1.000} & -- & 0.000 & -- & \best{0.000} \\
Stuck during diagnostic & \best{1.000} & \best{1.000} & \best{1.000} & 12.0 & \best{0.000} \\
Weak diagnostic response & \best{1.000} & \best{1.000} & \best{1.000} & 12.0 & \best{0.000} \\
\bottomrule
\end{tabular}
}
\end{table}

Table~\ref{tab:diagnostic_excitation} shows the implemented mitigation pathway for the stuck-at gap. A periodic diagnostic excitation pulse can expose a frozen plausible value when the measured process fails to move in the expected direction. Such pulses should be enabled only under conservative conditions: the trusted state is in band, no lockout or manual override is active, the diagnostic runtime or dose is bounded, and the post-command response window is logged. Without such an episode, an independent reference sample, or a real actuator-response record, AASVR-R still cannot guarantee detection of a plausible frozen value.
...
Code, configuration files, processed-data descriptors, paper-generation assets, and Docker workflow files will be supplied through a repository or archival record for the submission package. Raw hydroponic and external benchmark files are not committed during review. The authors plan to release or deposit the curated hydroponic sensor feed after credential removal and private-field review; if an archive DOI is assigned before publication, it should be cited in this statement and in the reference list. Timestamped actuator-command/state evidence is not available for the historical deployment because those logs were not retained, so the replay false-authorized-actuation results should not be interpreted as measured physical disturbances. Public third-party datasets can be obtained from their original providers, and restricted datasets must be obtained under the providers' terms.
