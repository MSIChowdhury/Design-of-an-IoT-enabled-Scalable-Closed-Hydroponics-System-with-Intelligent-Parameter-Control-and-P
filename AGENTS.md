...
- Treat `data/raw/hydroponic/agronomic_harvest.csv` as the optional authoritative per-plant harvest sheet for agronomic linkage. Use `scripts/22_prepare_agronomic.py --write-template` for the required schema, keep TNL/NL10 as counts, and interpret linkage outputs as mechanistic deployment context rather than causal yield evidence.
