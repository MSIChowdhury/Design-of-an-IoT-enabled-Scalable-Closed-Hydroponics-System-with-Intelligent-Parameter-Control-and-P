...
- 2026-05-31: Updated the ISA manuscript data-availability statement to state that all study data and code are available in the configured GitHub repository, while preserving anonymized wording in blinded files.
- 2026-09-24: Added a separate retrospective authorization reanalysis with archived historical artifacts, disjoint temporal source windows, six primary channels, independent surrogate-reference opportunities and event matching, elapsed-time cooldown/persistence, estimate-age reporting, validation coverage constraints, source-day bootstrap intervals, deterministic AASVR-R recovery checks, runtime measurements, tests, and generated report/figure commands. The compact run does not establish an AASVR advantage at comparable coverage; existing manuscript tables remain historical rather than being silently replaced. Also restored a missing AASVRConfig annotation import found by the full-repository lint check.
- 2026-09-24: Added and ran a separate decision-preserving telemetry feasibility study over complete chronological hydroponic blocks: six reporting policies, causal fixed-clock replay, network impairment simulation, stale/duplicate/reordered-message contracts, expiry-aware pending commands, locked coverage/error constraints, clustered intervals, and a real two-process UDP loopback probe. Edge command reporting substantially reduces modeled traffic; state snapshots increase traffic under the selected JSON protocol and exceed the declared error ceiling. No policy meets the joint requirements across all validation networks, so results remain a feasibility study rather than a new manuscript superiority claim.
- 2026-09-24: Implemented and ran bounded command-event retries, revision-specific receipt ACKs, cancellation tombstones, original-time expiration, and duplicate protection with ACK/cancellation ablations on the preserved telemetry populations. Validation selected two transmissions 32 seconds apart; the 95% coverage/1% undesirable-command criteria passed every evaluation profile, while the 99% validation target remained infeasible. ACK removal preserved measured outcomes with lower impaired-link traffic. Added reproducible reports, source-day paired intervals, a two-process UDP probe, provenance checks, and protocol tests; all 84 repository tests passed. Results remain adaptive retrospective command-fidelity evidence rather than physical control or safety validation.
- 2026-09-24: Reexecuted the complete reliable-events Make target (108 validation and 36 evaluation jobs) and all 84 tests on request. Reported replay outcomes and paired intervals reproduced unchanged; refreshed the real UDP loopback latency measurements in the results report. No algorithm or parameter changes were needed.
- 2026-09-24: Implemented and executed the frozen command-delivery challenge: 25 scenarios, four new network seeds, six sensor streams, and three fixed policies (1,800 replays), including a stronger same-receiver/no-ACK plain-repetition baseline. Added crossed day/seed intervals, phase/correlated-loss/clock/restart stress, structural output-violation counters, frozen fingerprints, six tests, and a managed two-container impaired-UDP historical replay (15 combinations). Candidate passed 16/25 point and 4/25 marginal interval criteria; cancellation's benefit was small, and restart/clock failures plus poor change-heavy-window UDP results remain explicit. Added protocol/results/assessment documents without retuning or rewriting manuscript claims; all 90 tests passed.
- 2026-09-24: Added a separate SQLite-backed recovery contract with durable pre-output reservations, explicit uncertain-execution blocking, clock-interval expiry/cooldown checks, and fresh-evidence barriers after restart/disconnection. Ran 936 comparative replays on new schedules plus 25 actual SIGKILL/disk-reopen cases and storage timing measurements. Preserved the first run before correcting a first-contact freshness edge case, then reran the unchanged suite with the correction disclosed. Final in-contract structural counts are zero for V2 versus current-protocol duplicate/expired/cooldown counts of 334/88/2208, but V2 fails every original availability target (49.94% ideal coverage) and uses 9.89× traffic; the out-of-bound clock case still fails expiry. Added protocol/results/assessment, Make target, and 17 tests; all 107 tests and lint passed. No physical exactly-once claim or automatic uncertain-state reset was added.
- 2026-09-24: Implemented and ran the frozen timing-feasibility/component study: 2,160 replays covering volatile, durable-only, clock/dispatch, and full-recovery stages; explicit B=0/1/4/8/16 sensitivity; and polling versus deadline-aware output wakeups with unchanged 16-second source/packet intake. Recorded and reconciled 1,241,280 per-command ledger entries, terminal reasons, admissible intervals, paired day/seed uncertainty, timelines, and within-contract empirical frontiers. Durable-only preserved 100% ideal-link replay coverage; deadline scheduling improved full-recovery B=16 ideal coverage from 49.94% to 66.73% at identical traffic, but no tested configuration meets all routine-profile coverage/error/structural criteria. Added protocol/results/assessment, Make target, and seven tests; all 114 tests and lint passed. Clock bounds remain unmeasured sensitivity assumptions, and output timing remains simulated.
- 2026-09-24: Implemented and ran a local three-container instrumented execution experiment with a durable mock actuator: 300 measured transactions plus 30 warmups, 90 shared-host clock exchanges, 15 actual process-kill/restart cases, and six independently confirmed output/ACK-loss cases that remained blocked without a second adapter invocation. Added paired measured-trace extrapolation over five frozen candidates (270 historical replays), compact snapshots, timing/coverage figures, provenance checks, and 17 tests; all 131 repository tests and lint passed. Durable polling retained 99.33% idle/contention coverage but fell to 1.57% under injected ACK loss because independent reconciliation remains unimplemented. Corrected the precollection clock-domain guard to compare shared kernel boot plus time-namespace offsets rather than namespace inode identity. Independent-device clock/outage bounds, physical execution, and power-loss durability remain unverified; no controller algorithms or manuscript claims were retuned.
- 2026-09-24: Implemented event-specific bounded reconciliation against an independent durable mock ledger, including persistent query budgets, command/stream/digest/clock binding, completion-time cooldown preservation, fresh-source barriers, and permanent cancellation fences that survive delayed requests. Ran 30 local UDP/recovery cases including six receiver SIGKILLs around reconciliation commit and three mock SIGKILLs with pending work; all passed. Ran 774 paired historical replays and exactly reproduced all 270 prior baseline rows. With available terminal evidence, durable polling coverage rose from 1.57% to 99.18% at 11.80% added modeled traffic; unavailable/mismatched/contradictory evidence remained blocked. Observed duplicate/expiry/cooldown violations were zero in this grid. Added protocol/results/assessment, compact snapshots, a Docker launcher, and 24 tests; all 155 tests and lint passed. Physical execution/record atomicity, independent clocks, authentication, and deployment safety remain unverified.
